...
func (l *DefaultLogger) logWithContext(ctx context.Context, level slog.Level, msg string, args ...any) {
	// 基本的な共通フィールドを自動で追加
	attrs := []slog.Attr{
		slog.Group("service",
...
func (l *DefaultLogger) WithError(ctx context.Context, msg string, err error, args ...any) {
...
func (l *DefaultLogger) LogOperation(ctx context.Context, operation string, duration time.Duration, success bool, args ...any) {
...
func (l *DefaultLogger) LogHTTPRequest(ctx context.Context, method, path string, status int, duration time.Duration, args ...any) {
	allArgs := append(args,
		"http_method", method,
		"http_path", path,
		"http_status", status,
...
func (l *DefaultLogger) LogBusinessEvent(ctx context.Context, event string, entityType string, entityID string, args ...any) {
	allArgs := append(args,
		"event", event,
