...
| US-SRE-HEALTH-IMPLEMENT-API              | SREとして、アプリケーションの基本的な健全性（APIサーバーの応答、DB接続性）を確認できるヘルスチェックAPI (`/health`) を実装したい                     | これにより、システムの死活監視を容易にし、主要な依存関係の問題を早期に検知できるようになる                                                 | M                  |
...
| ID                          | ユーザーストーリー                                                                                              | 理由                                                                                       | ストーリーポイント |
| :-------------------------- | :-------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------- | :----------------- |
| US-SEC-OBS-AUDIT-LOGS       | **(利用)** セキュリティ担当として、システム内の重要な操作ログを収集し、監査・分析できるようにしたい             | これにより、不正行為の追跡やインシデント原因究明、コンプライアンス要件への対応が可能になる | L                  |
| US-SEC-OBS-CHECK-COMPLIANCE | **(利用)** セキュリティ担当として、コンプライアンス要件に関連する設定やログを自動的にチェックできるようにしたい | これにより、継続的なコンプライアンス遵守状況の確認を効率化し、監査対応負荷を軽減できる     | L                  |
