...
```text
Week3 Day3の日次講義資料の**依頼文**をアーティファクトで作ってください
作るのは、依頼文です。
```

```text
AWS オブザーバビリティ学習用 eコマースアプリのWeek 3 - Day 3（Lambda関数とS3連携の実装）の
詳細な実装手順書を作成してください。更新した日次講義テンプレートの形式に沿って、
以下の構造で作成してください：
...
この日の学習では特にAWSのサーバーレス機能を活用した商品画像リサイズ機能の実装と、S3バケットとの連携方法に焦点を当てます。LocalStackを使ったローカル環境でのテスト方法も含めてください。
Lambda関数の基本構造、S3トリガーの設定、処理結果の保存、エラーハンドリングについて具体的な実装手順を示してください。

サーバーレスアーキテクチャやLambda、S3の基本概念も簡潔に解説し、イベント駆動型設計の基礎についても触れてください。
コードサンプルは実行可能な形で提供し、各ステップでの期待される出力や結果も含めてください。

参照資料:
- Week 3の週次概要資料
- learning-roadmap.mdの1.5.3節
- mvp-implementation-plan.mdの1.4.1.3節
- フェーズ1全体の概念と設計ガイド（特に1.6.3.2節サーバーレス基盤）

コードサンプルを記載するにあたり、`/home/yoichi/workspace/aws-observability-ecommerce/backend-image-processor`を確認したうえで整合性の取れたコードを記載してください。
アーティファクトで作成してください。
```
