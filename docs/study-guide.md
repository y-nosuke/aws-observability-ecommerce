...
3. 【手順】- 5-7つの具体的な実装ステップを含めてください
   ※重要:
   - プロジェクトの現状のコードベースを確認し、実際のコード構造や命名規則に沿ったサンプルコードを提供してください
   - ファイルやディレクトリの作成は`mkdir`や`touch`コマンドで示し、ソースコードやコンフィグファイルの内容は別途コードブロックで示してください
   - catコマンドでのファイル作成とコード表示を同時に行うスタイルは避けてください
   - 各手順では既存コードとの整合性を確保し、既存のファイル構造やコード規約に従ってください
...
具体的なコード例やコマンドは、プロジェクトの現状のコードベースと整合性のある実行可能な形で提供し、
